'''
Скрипт который вытаскивает всех делегаторов, которым должна идти выплата по токену
'''
...
def get_noah_balance(wallet_address):
    '''
    Возвращает баланс NOAH на кошельке
    '''
...
noah_balance = get_noah_balance(wallet.wallet_from['address'])
to_be_payed = float(noah_balance) * 0.95
...
multisend_list = create_multisend_list(total_delegated_coins, payed_delegators_list, to_be_payed, 'NOAH')
...
def print_info():
    ...
    print(f'''
    -------------------------------------------------
    Баланс кошелька: {noah_balance} NOAH
    Будет выплачено: {to_be_payed} NOAH

    В выплате участвуют:
    — {len(payed_delegators_list)} адресов
    — {total_delegated_coins} NOAH

    1 {settings.TOKEN} – это:
    ~ {1*100/total_delegated_coins} % от общей суммы
    ~ {(1*100/total_delegated_coins)*to_be_payed*0.01} NOAH
    --------------------------------------------------
    ''')
